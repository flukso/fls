...
Flukso.TypeView=Backbone.View.extend({el:"[menu=type]",initialize:function(){_.bindAll(this,"render","clickButton");this.model.bind("change:type",this.render);$("[menu=type]").click(this.clickButton);this.render()},render:function(){$("[menu=type] li a").removeClass("active");var a="[type="+this.model.get("type")+"]";$(a).button("toggle");return this},events:{},clickButton:function(a){Flukso.alertView.clear();this.model.set({type:$(a.target).attr("type")});this.model.set({cumul:!1});this.model.set({reloadChart:!0})}});
