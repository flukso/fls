...
0==a.get("show")&&(a=".avatar [uid="+a.get("uid")+"]",$(a).parent().addClass("grey-out"));this.collection.syncUids()},remove:function(a){a=a.target;var b=Number($(a).attr("uid")),c=this.collection.getByUid(b);Drupal.settings.me.uid!=b&&($(a).parent().remove(),this.collection.remove(c),this.collection.syncUids(),Flukso.chartState.set({reloadChart:!0,refreshData:!Flukso.chartState.get("refreshData")}))},show:function(a){a=a.target;var b=Number($(a).attr("uid"));b=this.collection.getByUid(b);b.set({show:!b.get("show")});
$(a).parent().toggleClass("grey-out");Flukso.chartState.set({reloadChart:!0})}});
Flukso.UserCtrlView=Backbone.View.extend({el:"#fluksonian-ctrl",initialize:function(){0==Drupal.settings.me.uid?($("#fluksonians").parent().remove(),$(this.el).parent().remove()):($(this.el).show(),_.bindAll(this,"add"),$("#fluksonian-show").addClass("active"),this.add())},add:function(){var a=this;$("#fluksonian-add").typeahead({source:function(a,c){return $.getJSON("/dash/user/autocomplete/"+c).success(function(b){a.process(b)})},property:"name",onselect:function(b){a.collection.getByUid(b.uid)||
...
this.GET)},GET:function(){function a(a){var b=Flukso.timeParams[this.get("interval")].resSec;this.set({data:_.map(a,function(a){a[0]=1E3*a[0]-b;return a}),fetching:!1})}function b(a,b,c){clearTimeout(this.get("timeoutId"));"timeout"==b&&Flukso.alertView.timeoutRt(this.get("function"))}if(Flukso.chartState.get("type")==this.get("type")&&Flukso.userCollect.getByUid(this.get("uid")).get("show")){this.set({fetching:!0,interval:Flukso.chartState.get("interval")},{silent:!0});a=_.bind(a,this);b=_.bind(b,
this);var c={version:this.get("version"),interval:Flukso.timeParams[this.get("interval")].interval,resolution:Flukso.timeParams[this.get("interval")].resolution,unit:"minute"==this.get("interval")?this.get("unitRt"):this.get("unit")};"minute"==this.get("interval")?null==this.get("localUrl")?Flukso.alertView.noRealtime():($.getJSON(this.get("localUrl")+this.get("id")+this.get("callback"),c).success(a).error(b),this.set({timeoutId:setTimeout(this.GET,1E3)})):$.getJSON(this.get("baseUrl")+this.get("id")+
...
Flukso.ChartView=Backbone.View.extend({initialize:function(){this.collection.bind("change:fetching",this.render)},render:function(){var a=this.map(function(a){return 1==a.get("fetching")?1:0});if(_.reduce(a,function(a,b){return a+b},0))return this;Flukso.chartState.set({init:!1});var b=Flukso.chartState.get("type"),c=Flukso.chartState.get("interval");if("minute"==c){a=Flukso.chartState.get("unitRt."+b);var f=!1,e=1}else a=Flukso.chartState.get("unit."+b),f=Flukso.chartState.get("cumul"),e=Flukso.unitPowerFactor[b][a]?
Flukso.unitPowerFactor[b][a]:Flukso.unitEnergyFactor[b][a][c];var d=this.filter(function(a){return a.get("type")==b&&a.get("interval")==c&&Flukso.userCollect.getByUid(a.get("uid")).get("show")&&_.find(a.get("data"),function(a){return"nan"!=a[1]})});d=_.map(d,function(a){function b(a){return a[0]<d?!1:!0}var d=_.last(a.get("data"))[0]-Flukso.timeParams[c].range;return{name:a.get("userName")+"."+a.get("function"),data:_.map(f?_.filter(a.get("data"),b):a.get("data"),function(a,b,c){"nan"==a[1]?a[1]=
null:(a[1]*=e,f&&0!=b&&(a[1]+=c[b-1][1]));return a}),step:!0,tooltip:{yDecimals:0}}});if(Flukso.chartState.get("reloadChart")){Flukso.chartState.set({reloadChart:!1});var g=$.extend(!0,{},Flukso.chartDefaults);g.xAxis.range=Flukso.timeParams[c].range;g.yAxis.title.text=a;g.series=d;Flukso.chart=new Highcharts.StockChart(g)}else _.each(d,function(a,b){Flukso.chart.series[b].setData(a.data)});return this}});
Flukso.ExportView=Backbone.View.extend({el:"#export-ctrl",initialize:function(){},render:function(){},events:{click:"clickButton"},clickButton:function(a){var b=Flukso.chartState.get("type"),c=Flukso.chartState.get("interval");"minute"==c?Flukso.chartState.get("unitRt."+b):Flukso.chartState.get("unit."+b);a=Flukso.sensorCollect.filter(function(a){return a.get("type")==b&&a.get("interval")==c&&a.get("uid")==Drupal.settings.me.uid});var f={fields:[""],data:_.map(a[0].get("data"),function(a){return[(new Date(a[0])).toISOString()]})};
_.each(a,function(a){f.fields.push(a.get("function"));_.each(a.get("data"),function(a,b,c){f.data[b].push("nan"==a[1]?"":a[1])})});var e=Papa.unparse(f);a=new Date;a="flukso-"+(a.getFullYear()+"-"+a.getMonth()+1+"-"+a.getDate()+"-"+a.getHours()+a.getMinutes()+a.getSeconds())+".csv";var d=new Blob([e],{type:"text/csv;charset=utf-8;"});navigator.msSaveBlob?navigator.msSaveBlob(d,a):(e=document.createElement("a"),void 0!==e.download&&(d=URL.createObjectURL(d),e.setAttribute("href",d),e.setAttribute("download",
a),e.style="visibility:hidden",document.body.appendChild(e),e.click(),document.body.removeChild(e)))}});Flukso.Router=Backbone.Router.extend({routes:{":type/:interval":"gotoChart"},initialize:function(){_.bindAll(this,"updateRoute");Flukso.chartState.bind("change",this.updateRoute)},updateRoute:function(){var a=Flukso.chartState.get("type"),b=Flukso.chartState.get("interval");this.navigate(a+"/"+b,!0)},gotoChart:function(a,b){Flukso.chartState.set({type:a,interval:b,reloadChart:!0})}});
...
Flukso.userCtrlView=new Flukso.UserCtrlView({collection:Flukso.userCollect});Flukso.exportView=new Flukso.ExportView({collection:Flukso.sensorCollect});Flukso.router=new Flukso.Router;Backbone.history.start({root:"/dash"});Flukso.router.updateRoute()});