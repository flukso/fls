...
%%%
%% @doc Flukso API: /user/xyz/device endpoint

-module(api_user_device).
-author('Bart Van Der Meerssche <bart.vandermeerssche@flukso.net>').
...
to_json(ReqData, #state{uid = Uid, jsonp = Jsonp} = State) ->
    {data, Result} = mysql:execute(pool, user_device, [Uid]),

    Data = [{struct, [{<<"serial">>, list_to_binary(["FL",
                            string:right(integer_to_list(Serial), 8, $0)])},
                      {<<"device">>, Device},
                      {<<"version">>, Version},
                      {<<"heartbeat">>, Heartbeat},
                      {<<"uptime">>, Uptime},
                      {<<"resets">>, Resets}]}
              || [Device, Serial, Heartbeat, Version, Resets, Uptime]
